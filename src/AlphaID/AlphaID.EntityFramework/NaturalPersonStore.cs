...
internal class NaturalPersonStore(IdSubjectsDbContext dbContext) : NaturalPersonStoreBase
{
    ...
    {
        return dbContext.People.AsNoTrackingWithIdentityResolution()
            .SingleOrDefaultAsync(p => p.Id == person.Id, cancellationToken);
    }

    public override async Task<NaturalPerson?> FindByIdAsync(string userId, CancellationToken cancellationToken)
    {
        return await dbContext.People.FindAsync([userId], cancellationToken);
    }
    ...
    public override async Task<NaturalPerson?> FindByLoginAsync(string loginProvider,
        string providerKey,
        CancellationToken cancellationToken)
    {
        NaturalPersonLogin? login = await dbContext.PersonLogins.Include(naturalPersonLogin => naturalPersonLogin.User)
    ...
    {
        return dbContext.People.SingleOrDefaultAsync(u => u.NormalizedUserName == normalizedUserName,
            cancellationToken);
    }

    public override async Task<IList<Claim>> GetClaimsAsync(NaturalPerson user, CancellationToken cancellationToken)
    {
        return await dbContext.PersonClaims.Where(c => c.UserId == user.Id)
            .Select(c => new Claim(c.ClaimType, c.ClaimValue)).ToListAsync(cancellationToken);
    }

    public override async Task<IList<UserLoginInfo>> GetLoginsAsync(NaturalPerson user,
        CancellationToken cancellationToken)
    {
    ...
    public override async Task<string?> GetTokenAsync(NaturalPerson user,
        string loginProvider,
        string name,
        CancellationToken cancellationToken)
    {
        NaturalPersonToken? token = await dbContext.PersonTokens.FirstOrDefaultAsync(
            t => t.UserId == user.Id && t.LoginProvider == loginProvider && t.Name == name, cancellationToken);
        return token?.Value;
    }

    public override async Task<IList<NaturalPerson>> GetUsersForClaimAsync(Claim claim,
        CancellationToken cancellationToken)
    {
        IQueryable<NaturalPerson> query = from userClaim in dbContext.PersonClaims
    ...
    public override async Task RemoveClaimsAsync(NaturalPerson user,
        IEnumerable<Claim> claims,
        CancellationToken cancellationToken)
    {
        foreach (Claim claim in claims)
        {
            List<NaturalPersonClaim> matchedClaims = await dbContext.PersonClaims
                .Where(uc => uc.UserId.Equals(user.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type)
                .ToListAsync(cancellationToken);
            foreach (NaturalPersonClaim c in matchedClaims) dbContext.PersonClaims.Remove(c);
        }
    }

    public override async Task RemoveLoginAsync(NaturalPerson user,
        string loginProvider,
        string providerKey,
        CancellationToken cancellationToken)
    {
        NaturalPersonLogin? login = await dbContext.PersonLogins.FirstOrDefaultAsync(
            l => l.UserId == user.Id && l.LoginProvider == loginProvider && l.ProviderKey == providerKey,
            cancellationToken);
        if (login != null) dbContext.PersonLogins.Remove(login);
    }

    public override async Task RemoveTokenAsync(NaturalPerson user,
        string loginProvider,
        string name,
        CancellationToken cancellationToken)
    {
    ...
    public override async Task ReplaceClaimAsync(NaturalPerson user,
        Claim claim,
        Claim newClaim,
        CancellationToken cancellationToken)
    {
        List<NaturalPersonClaim> matchedClaims = await dbContext.PersonClaims
            .Where(uc => uc.UserId.Equals(user.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type)
            .ToListAsync(cancellationToken);
        foreach (NaturalPersonClaim matchedClaim in matchedClaims)
        {
    ...
    public override async Task SetTokenAsync(NaturalPerson user,
        string loginProvider,
        string name,
        string? value,
        CancellationToken cancellationToken)
