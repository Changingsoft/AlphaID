    ...
    OAuth2Service oAuth2Service)
{
    ...
    {
        WechatLoginSession session = await FindAsync(sessionId) ?? throw new ArgumentException("会话无效。");

        //向federal.changingsoft.com发起资源所有者密码凭据授予流（ROPC），拿取访问WebAPI的令牌
        OAuth2Result? oAuth2Result = await oAuth2Service.GetResourceOwnerPasswordCredentialTokenAsync(session.ClientId,
            session.ClientSecret, session.WechatUser!.UserPrincipalName, session.WechatUser.UserSecret,
            session.Resource);
        ...
        string queryString = await new FormUrlEncodedContent(queryData).ReadAsStringAsync();
