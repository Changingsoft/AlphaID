           ...
           logger)
{
    ...
    public virtual async Task<NaturalPerson?> FindByMobileAsync(string mobile, CancellationToken cancellation)
    {
        ...
        var person = await Store.FindByPhoneNumberAsync(phoneNumberString, cancellation);
        return person;
    }
    ...
    {
        var aggregator = new NaturalPersonCreateInterceptorAggregator(Interceptors.OfType<INaturalPersonCreateInterceptor>());
    ...
    public virtual Task AccessSuccededAsync(NaturalPerson person, string authenticationMethod)
    {
        ...
        Logger.LogInformation("用户{person}成功执行了登录，登录成功计数器+1，记录登录时间{time}，登录方式为：{authenticationMethod}", person, TimeProvider.GetUtcNow(), authenticationMethod);
    ...
    public override async Task<IdentityResult> DeleteAsync(NaturalPerson user)
    {
        ...
        foreach (var interceptor in Interceptors.OfType<INaturalPersonDeleteInterceptor>())
        {
    ...
    public override async Task<IdentityResult> ChangePasswordAsync(NaturalPerson user, string currentPassword, string newPassword)
    {
        ...
        if (Options.Password.MinimumAge > 0)
        {
            ...
            {
                var coldDownEnd = TimeProvider.GetUtcNow()
    ...
    public override async Task<IdentityResult> ResetPasswordAsync(NaturalPerson user, string token, string newPassword)
    {
        ...
        if (!result.Succeeded)
        {
            await EventService.RaiseAsync(new ChangePasswordFailureEvent(result.Errors.Select(e => e.Description).Aggregate((x, y) => $"{x},{y}")));
            return result;
        }
