...
public static class RadiusAttributeStructExtensions
{
    ...
    public static byte[] ToByteArray(this RadiusAttributeStruct attribute)
    {
        ...
        Marshal.Copy(attribute.ValuePtr, valueBytes, 0, valueBytes.Length);
    ...
    public static RadiusAttributeStruct FromByteArray(byte[] data)
    {
        ...
        RadiusAttributeStruct attribute = new RadiusAttributeStruct
        {
            Type = data[0],
            Length = data[1],
            ValuePtr = Marshal.AllocHGlobal(data.Length - 2)
        };

        Marshal.Copy(data, 2, attribute.ValuePtr, data.Length - 2);
    ...
    {
        if (attribute.ValuePtr != IntPtr.Zero)
        {
            Marshal.FreeHGlobal(attribute.ValuePtr);
            attribute.ValuePtr = IntPtr.Zero;
        }
    }
}